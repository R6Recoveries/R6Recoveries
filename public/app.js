...
/* =========================
R6R VISUAL SYSTEM — presentation only
========================= */
(function initR6RVisualSystem(){
  function start(){
    const boot=document.getElementById('bootSequence');
    if(boot){
      const bootText=document.getElementById('bootText');
      const seen=sessionStorage.getItem('r6r_boot_seen');
      if(seen){ boot.classList.add('done'); }
      else{
        const steps=['INITIALIZING R6R SYSTEM','LINKING ARMORY MODULES','SYSTEM SCAN ONLINE','ACCESS GRANTED'];
        let i=0;
        const timer=setInterval(()=>{ i++; if(bootText && steps[i]) bootText.textContent=steps[i]; if(i>=steps.length-1){clearInterval(timer);setTimeout(()=>{boot.classList.add('done');sessionStorage.setItem('r6r_boot_seen','1')},420)} },330);
      }
    }
    document.querySelectorAll('[data-scan]').forEach(node=>node.addEventListener('click',()=>{
      document.querySelectorAll('[data-scan]').forEach(n=>n.classList.remove('active'));
      node.classList.add('active');
      const title=document.getElementById('scanTitle'); const copy=document.getElementById('scanCopy');
      if(title) title.textContent=node.dataset.scan||'SYSTEM NODE';
      if(copy) copy.textContent=node.dataset.copy||'';
    }));
    document.querySelectorAll('[data-field-test]').forEach(card=>card.addEventListener('click',()=>{
      showToast('Field Test bay ready — add your real video clip here.');
    }));
    const observer=new IntersectionObserver(entries=>entries.forEach(entry=>{if(entry.isIntersecting) entry.target.classList.add('inView')}),{threshold:.12});
    document.querySelectorAll('.section').forEach(section=>{section.classList.add('revealSection');observer.observe(section)});
  }
